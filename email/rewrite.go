...
func rewrite(w io.Writer, mp *messagePart, groupAddress, personName, personAddress string) (err error) {
	...
	if mp.plain {
		if _, err = fmt.Fprintf(w, "________\r\nThis message was sent via the %s@SunnyvaleSERV.org mailing list to %s <%s>.\r\nTo unsubscribe, visit https://SunnyvaleSERV.org/unsubscribe/%s.\r\n", groupAddress, personName, personAddress, personAddress); err != nil {
			return err
		}
	}
	if mp.html {
		if _, err = fmt.Fprintf(w, "<div style=\"height:1em;width:5em;border-bottom:1px solid #888\"></div><div style=\"color:#888\">This message was sent via the <a style=\"color:#888\">%s@SunnyvaleSERV.org</a> mailing list to %s &lt;<a style=\"color:#888\">%s</a>&gt;.<br>To unsubscribe, visit <a style=\"color:#888\" href=\"https://SunnyvaleSERV.org/unsubscribe/%[2]s\">https://SunnyvaleSERV.org/unsubscribe/%[2]s</a>.</div>", groupAddress, html.EscapeString(personName), html.EscapeString(personAddress)); err != nil {
			return err
		}
	}
