<!--
Role2Edit displays the page for creating and editing roles.
-->
...
#role2
  ...
  template(v-else)
    SForm(:submitLabel='submitLabel', :disabled='submitting', @submit='onSubmit')
      ...
      SFRadioGroup#role2-org(label='Org', :options='orgOptions', v-model='role.org')
      SFRadioGroup#role2-privLevel(
        label='Priv Level',
        :options='role.org ? privLevelOptions : privLevelNoOption',
        v-model='role.privLevel'
      )
      SFCheckGroup#role2-flags(label='Flags', :options='flagOptions', v-model='flags')
      SFCheckGroup#role2-implies(
      ...
      label#role2-lists-label.form-item-label Lists
      .form-item-input2
      ...
      Modal(ref='listEditModal', v-slot='{ close }')
        SForm(
          ...
          SFRadioGroup#role2-listEdit-subModel(
            label='Subscription',
            :options='subModelOptions',
            v-model='editingList.subModel'
          )
          SFCheck#role2-listEdit-sender(label='Can Send', v-model='editingList.sender')
      MessageBox(
        ref='deleteModal',
        title='Delete Role',
        cancelLabel='Keep',
...
export default defineComponent({
  ...
  setup() {
    ...
    axios.get<GetRole>(`/api/roles2/${route.params.rid}`).then((resp) => {
    ...
    async function onSubmit() {
      ...
      const resp = (await axios.post<PostRole>(`/api/roles2/${route.params.rid}`, body)).data
    ...
    async function onDelete() {
      if (deleteModal.value) {
        ...
        if (confirmed) {
          submitting.value = true
          await axios.delete(`/api/roles2/${route.params.rid}`)
          submitting.value = false
          router.push('/admin/roles2')
        }
      }
