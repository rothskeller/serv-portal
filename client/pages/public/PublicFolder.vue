<!--
PublicSubFolder displays the contents of a public folder.
-->
...
PublicPage(title='Sunnyvale SERV')
  ...
  #public-sf(v-else)
    ...
    .public-sf-line
      ...
      b-link.public-sf-name(:to='parentURL', v-text='parentName')
    .public-sf-line(v-for='child in folder.children', :key='`f${child.id}`')
      ...
      .public-sf-name
        b-link(:to='child.url') {{ child.name }}
...
export default {
  ...
  computed: {
    parentName() {
      if (!this.folder || !this.folder.parent || !this.folder.parent.url) return 'Home Page'
      return this.folder.parent.name
    },
    parentURL() {
      if (!this.folder || !this.folder.parent || !this.folder.parent.url) return '/'
      return this.folder.parent.url
    },
  },
  created() {
    this.loadFolder()
