...
div(v-if="!folder")
  b-spinner(small)
div#files(v-else)
  .files-line(v-if="folder.id" @click="onMove(folder.parent ? folder.parent.id : 0)")
    svg.files-icon(xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512")
  ...
  .files-line(v-for="child in folder.children" :key="`f${child.id}`" @click="onMove(child.id)")
    svg.files-icon(xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512")
  ...
  .files-line(v-for="doc in folder.documents" :key="`d${doc.id}`" @click="onOpen(doc.id)")
    svg.files-icon(v-if="doc.name.endsWith('.docx') || doc.name.endsWith('.doc')" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 384 512")
    ...
    svg.files-icon(v-else-if="doc.name.endsWith('.pdf')" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 384 512")
    ...
    svg.files-icon(v-else-if="doc.name.endsWith('.ppt') || doc.name.endsWith('.pptx')" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 384 512")
    ...
    svg.files-icon(v-else-if="doc.name.endsWith('.xls') || doc.name.endsWith('.xlsx')" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 384 512")
    ...
    svg.files-icon(v-else-if="doc.name.endsWith('.jpg') || doc.name.endsWith('.jpeg')" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 384 512")
    ...
    svg.files-icon(v-else xmlns="http://www.w3.org/2000/svg" viewBox="0 0 384 512")
    ...
    b-link.files-name(:to="`/api/folders/${folder.id}/${doc.id}`") {{doc.name}}
</template>
...
export default {
  ...
  methods: {
    ...
    onOpen(docID) { window.location = `/api/folders/${this.folder.id}/${docID}` },
  },
}
</script>
