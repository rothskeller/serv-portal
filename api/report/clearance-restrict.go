...
func matchClearanceRestriction(p *model.Person, s string) bool {
	...
	case "dswCERT":
		return p.DSWRegistrations != nil &&
			...
			(p.DSWUntil == nil || p.DSWUntil[model.DSWCERT].IsZero() || !p.DSWUntil[model.DSWCERT].Before(time.Now()))
	case "dswComm":
		return p.DSWRegistrations != nil &&
			!p.DSWRegistrations[model.DSWComm].IsZero() &&
			!p.DSWRegistrations[model.DSWComm].After(time.Now()) &&
			(p.DSWUntil == nil || p.DSWUntil[model.DSWComm].IsZero() || !p.DSWUntil[model.DSWComm].Before(time.Now()))
	case "idPhoto":
