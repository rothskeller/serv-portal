...
func StrongPassword(p *model.Person, password string) bool {
	...
	if p != nil {
		hints = append(hints, p.InformalName, p.FormalName, p.CallSign, p.Email, p.Email2, p.HomeAddress.Address, p.MailAddress.Address, p.WorkAddress.Address, p.CellPhone, p.HomePhone, p.WorkPhone)
	}
