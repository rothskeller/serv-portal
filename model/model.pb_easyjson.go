...
func easyjson6578def1EncodeSunnyvaleservOrgPortalModel9(out *jwriter.Writer, in Person) {
	...
	}
	{
		...
		if in.DSWRegistrations == nil && (out.Flags&jwriter.NilMapAsEmpty) == 0 {
			...
			out.RawByte('{')
			v40First := true
			for v40Name, v40Value := range in.DSWRegistrations {
				if v40First {
					v40First = false
				} else {
					out.RawByte(',')
				}
				out.IntStr(int(v40Name))
				out.RawByte(':')
				out.Raw((v40Value).MarshalJSON())
			}
			out.RawByte('}')
		}
	}
	{
		...
		if in.DSWUntil == nil && (out.Flags&jwriter.NilMapAsEmpty) == 0 {
			...
			out.RawByte('{')
			v41First := true
			for v41Name, v41Value := range in.DSWUntil {
				if v41First {
					v41First = false
				} else {
					out.RawByte(',')
	...
	if len(in.Roles) != 0 {
		...
		out.RawString(prefix)
		{
			out.RawByte('{')
			v42First := true
			for v42Name, v42Value := range in.Roles {
				if v42First {
					v42First = false
				} else {
					out.RawByte(',')
		...
		case "lists":
			if in.IsNull() {
				...
				for !in.IsDelim(']') {
					...
					out.Lists = append(out.Lists, v47)
					in.WantComma()
				}
				in.Delim(']')
			}
		...
		case "people":
			if in.IsNull() {
				...
				for !in.IsDelim('}') {
					...
					in.WantColon()
					var v50 ListPersonStatus
					v50 = ListPersonStatus(in.Uint8())
					(out.People)[key] = v50
					in.WantComma()
				}
				in.Delim('}')
...
func easyjson6578def1EncodeSunnyvaleservOrgPortalModel12(out *jwriter.Writer, in List) {
	...
	if len(in.People) != 0 {
		...
		}
		{
			out.RawByte('{')
			v51First := true
			for v51Name, v51Value := range in.People {
				if v51First {
					v51First = false
				} else {
					out.RawByte(',')
		...
		case "shifts":
			if in.IsNull() {
				...
				for !in.IsDelim(']') {
					...
					out.Shifts = append(out.Shifts, v53)
					in.WantComma()
				}
				in.Delim(']')
			}
...
func easyjson6578def1DecodeSunnyvaleservOrgPortalModel14(in *jlexer.Lexer, out *BackgroundCheck) {
	isTopLevel := in.IsStart()
	if in.IsNull() {
		if isTopLevel {
			in.Consumed()
...
func easyjson6578def1EncodeSunnyvaleservOrgPortalModel14(out *jwriter.Writer, in BackgroundCheck) {
	out.RawByte('{')
	first := true
	_ = first
	if in.Date != "" {
