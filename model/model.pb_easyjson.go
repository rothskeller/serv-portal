...
func easyjson6578def1EncodeSunnyvaleservOrgPortalModel4(out *jwriter.Writer, in TextMessage) {
	...
	if len(in.Recipients) != 0 {
		...
		{
			out.RawByte('[')
			for v12, v13 := range in.Recipients {
				if v12 > 0 {
					out.RawByte(',')
				}
				if v13 == nil {
		...
		case "people":
			if in.IsNull() {
				...
				for !in.IsDelim(']') {
					var v21 PersonID
					v21 = PersonID(in.Int())
					out.People = append(out.People, v21)
					in.WantComma()
				}
				in.Delim(']')
			}
...
func easyjson6578def1EncodeSunnyvaleservOrgPortalModel6(out *jwriter.Writer, in Role2) {
	...
	if len(in.Implies) != 0 {
		...
		}
		{
			out.RawByte('{')
			v22First := true
			for v22Name, v22Value := range in.Implies {
				if v22First {
					v22First = false
				} else {
					out.RawByte(',')
	...
	if len(in.Lists) != 0 {
		...
		}
		{
			out.RawByte('{')
			v23First := true
			for v23Name, v23Value := range in.Lists {
				if v23First {
					v23First = false
				} else {
					out.RawByte(',')
	...
	for !in.IsDelim('}') {
		...
		case "notes":
			if in.IsNull() {
				...
				for !in.IsDelim(']') {
					...
					out.Notes = append(out.Notes, v27)
					in.WantComma()
				}
				in.Delim(']')
			}
		...
		case "orgs":
			if in.IsNull() {
				...
				for !in.IsDelim(']') {
					var v31 OrgMembership
					(v31).UnmarshalEasyJSON(in)
					out.Orgs = append(out.Orgs, v31)
					in.WantComma()
				}
				in.Delim(']')
			}
...
func easyjson6578def1EncodeSunnyvaleservOrgPortalModel9(out *jwriter.Writer, in Person) {
	...
	}
	{
		...
		if in.DSWRegistrations == nil && (out.Flags&jwriter.NilMapAsEmpty) == 0 {
			...
			out.RawByte('{')
			v36First := true
			for v36Name, v36Value := range in.DSWRegistrations {
				if v36First {
					v36First = false
				} else {
					out.RawByte(',')
	...
	}
	{
		...
		if in.DSWUntil == nil && (out.Flags&jwriter.NilMapAsEmpty) == 0 {
			...
			out.RawByte('{')
			v37First := true
			for v37Name, v37Value := range in.DSWUntil {
				if v37First {
					v37First = false
				} else {
					out.RawByte(',')
				}
				out.IntStr(int(v37Name))
				out.RawByte(':')
				out.Raw((v37Value).MarshalJSON())
			}
			out.RawByte('}')
		}
	}
	...
	if len(in.Roles) != 0 {
		...
		out.RawString(prefix)
		{
			out.RawByte('{')
			v38First := true
			for v38Name, v38Value := range in.Roles {
				if v38First {
					v38First = false
				} else {
					out.RawByte(',')
				}
				out.IntStr(int(v38Name))
				out.RawByte(':')
		...
		case "no_email":
			if in.IsNull() {
				...
				for !in.IsDelim(']') {
					var v46 PersonID
					v46 = PersonID(in.Int())
					out.NoEmail = append(out.NoEmail, v46)
					in.WantComma()
				}
				in.Delim(']')
			}
		case "no_text":
			if in.IsNull() {
				...
				for !in.IsDelim(']') {
					var v47 PersonID
					v47 = PersonID(in.Int())
					out.NoText = append(out.NoText, v47)
					in.WantComma()
				}
				in.Delim(']')
			}
...
func easyjson6578def1DecodeSunnyvaleservOrgPortalModel14(in *jlexer.Lexer, out *Event) {
	isTopLevel := in.IsStart()
	if in.IsNull() {
		if isTopLevel {
			in.Consumed()
...
func easyjson6578def1EncodeSunnyvaleservOrgPortalModel14(out *jwriter.Writer, in Event) {
	out.RawByte('{')
	first := true
	_ = first
	if in.ID != 0 {
