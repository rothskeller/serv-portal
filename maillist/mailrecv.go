...
func ReceivedHook(r *request.Request) {
	...
	if sesID = r.FormValue("sesID"); sesID == "" {
		sendError(r, "400 sesID Required", http.StatusBadRequest)
		return
	}
	if sesHash = r.FormValue("sesHash"); sesHash == "" {
		sendError(r, "400 sesHash Required", http.StatusBadRequest)
		return
	}
	if sesHash != private.ComputeHash(sesID) {
		sendError(r, "403 Incorrect Hash", http.StatusForbidden)
		return
	}
	if raw, _, err = r.FormFile("message"); err != nil || raw == nil {
		sendError(r, "400 message Required", http.StatusBadRequest)
		return
	}
	// Open a temporary file and read the message into it.
	if mf, err = os.CreateTemp(".", ""); err != nil {
		sendError(r, "500 CreateTemp "+err.Error(), http.StatusInternalServerError)
		return
	}
	defer os.Remove(mf.Name())
	if _, err = io.Copy(mf, raw); err != nil {
		sendError(r, "500 copy message "+err.Error(), http.StatusInternalServerError)
		return
	}
	// Get the message ID from the message.
	if _, err = mf.Seek(0, 0); err != nil {
		sendError(r, "500 rewind message "+err.Error(), http.StatusInternalServerError)
		return
	}
	if msg, err = mail.ReadMessage(mf); err != nil {
		sendError(r, "400 invalid message "+err.Error(), http.StatusBadRequest)
		return
	...
	if err = osdep.WriteLock(tfh); err != nil {
		sendError(r, "500 lock tracking file "+err.Error(), http.StatusInternalServerError)
		return
	}
	...
	// If the tracking file didn't already exist, save the message file
	// before unlocking the tracking file.
	if !existed {
		if err = os.Rename(mf.Name(), qfn); err != nil {
			sendError(r, "500 move message "+err.Error(), http.StatusInternalServerError)
			return
		}
	}
	if err = tfh.Close(); err != nil {
		sendError(r, "500 close tracking file "+err.Error(), http.StatusInternalServerError)
		return
	}
	// Start the routemail program in the background to handle it.
	cmd := exec.Command(`C:\serv\bin\routemail.exe`)
	if err = cmd.Start(); err != nil {
		sendError(r, "500 start routemail "+err.Error(), http.StatusInternalServerError)
		return
	}
	// All is well.
...
func sendError(r *request.Request, estr string, code int) {
	r.LogEntry.Problems.Add(estr)
	http.Error(r, estr, code)
}
