...
func markActive(tx *store.Tx, loginID string) {
	...
	for _, p := range tx.FetchPeople() {
		if p.VolgisticsID != 0 {
			if p.HasPrivLevel(model.PrivMember) {
	...
	markInactive(tx, loginID, inactiveKeys)
}

func setStatusToActive(loginID, name string, volunteerID int) {
	var (
		key string
		val url.Values
...
func markInactive(tx *store.Tx, loginID string, inactiveKeys map[string]bool) {
	var (
		val  url.Values
		doc  *goquery.Document
