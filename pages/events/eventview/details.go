...
func showDetails(r *request.Request, main *htmlb.Element, user *person.Person, e *event.Event, ts []*task.Task) {
	...
	date := datet.Format("Monday, January 2, 2006")
	if e.Start()[11:] != "00:00" || e.End()[11:] != "00:00" {
		if e.Start() != e.End() {
			bdiv.E("div class=eventviewDetailsTime>%s from %s to %s", date, e.Start()[11:], e.End()[11:])
		} else {
			bdiv.E("div class=eventviewDetailsTime>%s at %s", date, e.Start()[11:])
		}
	} else {
	...
	if e.Venue() != 0 {
		...
		bdiv.E("div class=eventviewDetailsVenue>Location TBD")
	}
	if e.Details() != "" {
